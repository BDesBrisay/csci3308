...
$(document).ready(() => {
    Object.keys(states).map((state) => {
        ...
        const url = `https://api.forecast.io/forecast/7b1618dd2d9aa7c62c14d9324df356b3/${info.lat},${info.lng}`;
        $.ajax({ url, dataType: 'jsonp' }).then((data) => {
            const temp = data.currently.temperature;
            $(`#${state}`).css('fill', getColor(temp));
        });
    });
});